#include "damapretacoroada.h"
#include "tabdama.h"
#include <QStandardPaths>
#include "imagensdama.h"

damaPretaCoroada::damaPretaCoroada(int px, int py):pecasDama(px,py)
{
    setTurno(false);
    setTipo(2);
}

void damaPretaCoroada::criaPeca(QPainter &painter,tabDama *tab)
{

    /*QString caminho = QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation);
    caminho.append("/ImagensTrabalho/DamaPretaCoroada.png");*/
    ...
    painter.drawImage(QRect(tab->getPosicao()+tab->getPosicao()*getPosicaoX(),tab->getPosicao()+tab->getPosicao()*getPosicaoY(),tab->getPosicao()
                            ,tab->getPosicao()), QImage(img_damaPretaCoroada));
}

void damaPretaCoroada::movimento(QPainter &painter, tabDama *tab)
{
...
bool damaPretaCoroada::podeIr(int x, int y, tabDama *tab)
{
