...
damaVermelha::damaVermelha(int px, int py):pecasDama(px,py)
{
    setTurno(false);
    setTipo(0);
}

void damaVermelha::criaPeca(QPainter &painter,tabDama *tab)
{
   /* QString caminho = QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation);
    caminho.append("/ImagensTrabalho/DamaVermelha.png");*/
    ...
    painter.drawImage(QRect(tab->getPosicao()+tab->getPosicao()*getPosicaoX(),tab->getPosicao()+tab->getPosicao()*getPosicaoY(),tab->getPosicao()
                            ,tab->getPosicao()), QImage(img_damaVermelha));
}

void damaVermelha::movimento(QPainter &painter, tabDama *tab)
{
...
bool damaVermelha::podeIr(int x, int y, tabDama *tab)
{
